...
module.exports = function (data, dest, callback, cb){
  ...
  geodata.meta = {'collection':'geodata',
          'headers':['ID','User', 'UserId','Created At', 'Closed At','Min Lat', 'Min Lon', 'Max Lat', 'Max Lon', 'BBOX', 'Comment']}
      ...
      parseString(xml, function (err, result, cb) {
        ...
        if (result) {
          bundle.push( [
            ...
            [ 
              "[" + result.osm.changeset[0]['$'].min_lon,result.osm.changeset[0]['$'].min_lat + "]",
              "[" + result.osm.changeset[0]['$'].min_lon,result.osm.changeset[0]['$'].max_lat + "]",
              "[" + result.osm.changeset[0]['$'].max_lon,result.osm.changeset[0]['$'].max_lat + "]",
              "[" + result.osm.changeset[0]['$'].max_lon,result.osm.changeset[0]['$'].min_lat + "]",
              "[" + result.osm.changeset[0]['$'].min_lon,result.osm.changeset[0]['$'].min_lat + "]",
            ],
            result.osm.changeset[0]['tag'][0]['$']['v']
          ])
        }
      })
